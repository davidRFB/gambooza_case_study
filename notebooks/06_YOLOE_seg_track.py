...
def parse_args():
    ...
    # Model / detection
    p.add_argument("--model", default="yoloe-11l-seg.pt", help="YOLOE segmentation model weights.")
    p.add_argument(
        "--classes",
        nargs="+",
        default=["cup", "glass", "beer glass", "person"],
    ...
    p.add_argument(
        ...
        type=float,
        default=0.15,
        help="Minimum detection confidence (lower than bbox-only since masks give more signal).",
    )
...
def main():
    ...
    if args.tap_divider:
        ...
        if "tap_divider" in data:
            ...
            print(f"TAP_DIVIDER loaded from {roi_json}: {tuple(round(v, 4) for v in tap_divider)}")
    ...
    if tap_divider is None:
        print(
            "WARNING: No tap divider defined. All pours will be unclassified.\n"
    ...
    if tap_divider:
        ...
        ax.plot(
            [lx1, lx2], [ly1, ly2], color="red", linewidth=2, linestyle="--", label="A | B divider"
        )
        mid_x = (lx1 + lx2) / 2
        ax.text(
            mid_x / 2,
    ...
    print(f"\nTracking video every {args.sample_every} frame(s) with {args.tracker} ...")
    cap = cv2.VideoCapture(str(args.video))
    frame_idx = 0
    all_detections = []
    ...
    if args.record_range:
        ...
        print(
            f"Recording annotated video: {args.record_range[0]:.1f}s -> "
    ...
    while True:
        ...
        if frame_idx % args.sample_every == 0:
            ...
            results = model.track(
                crop, persist=True, tracker=args.tracker, conf=args.conf_threshold, verbose=False
            )[0]
            ...
            if has_boxes:
                ...
                for i in range(n_det):
                    ...
                    if has_masks and i < len(results.masks.data):
                        ...
                        mean_r, mean_g, mean_b, mean_h, mean_s, mean_v = mask_pixel_stats(
                            crop, mask_bool
                        )
            ...
            if should_save_frame:
                ...
                if div_px:
                    cv2.line(
                        annotated, (div_px[0], div_px[1]), (div_px[2], div_px[3]), (0, 0, 255), 2
                    )
                cv2.putText(
                    annotated,
    ...
    cup_tracks = {tid: td for tid, td in track_data.items() if td["class"] in CUP_CLASSES}
    ...
    for tid in sorted(cup_tracks.keys()):
        ...
        print(
            f"  ID {tid:3d} [{td['class']:>10s}] [Tap {td['tap']}]: "
    ...
    if cup_tracks:
        ...
        fig, axes = plt.subplots(n_tracks, 3, figsize=(18, max(4, n_tracks * 3)), squeeze=False)
    ...
    if cup_tracks:
        ...
        if transitions:
            ...
            for tid, tr in transitions.items():
                td = cup_tracks[tid]
                print(
                    f"  ID {tid} [{td['class']}] [Tap {td['tap']}] at {tr['time_s']:.1f}s  "
        ...
        fig, axes = plt.subplots(n_cup, 1, figsize=(14, max(4, n_cup * 3)), squeeze=False)
    ...
    if transitions:
        ...
        if grid_images:
            ...
            axes_flat = (
                np.array(axes).flatten() if n_rows > 1 else ([axes] if n_cols == 1 else list(axes))
            )
    ...
    summary = {
        ...
        "fill_transitions": {str(tid): tr for tid, tr in transitions.items()}
        if transitions
        else {},
    }
