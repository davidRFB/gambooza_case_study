...
def parse_args():
    ...
    p.add_argument(
        "--input", required=True, type=Path, help="raw_detections.csv from 03_YOLO_track.py"
    )
...
def build_incompatibility(tids: list[int], info: dict, threshold: int) -> set[tuple[int, int]]:
...
def greedy_coexistence_groups(
    ...
    for tid in sorted_tids:
        ...
        for gi, group in enumerate(groups):
            if all((min(tid, other), max(tid, other)) not in incompat for other in group):
        ...
        if not compatible_gi:
            ...
            for gi in compatible_gi:
                ...
                d = np.hypot(t["median_cx"] - centroid_x, t["median_cy"] - centroid_y)
                if d < best_dist:
                    best_dist = d
                    best_gi = gi
...
def render_annotated_video(
    ...
    logger.info(
        "Rendering annotated video for %d segment(s) (%d frames total)",
        len(frame_ranges),
        len(needed_frames),
    )
...
def run_relink(
    ...
    for tid in tids:
        t = info[tid]
        logger.debug(
            "  Track %3d: frames %4d-%4d  dets=%4d  median=(%.0f, %.0f)",
            tid,
            t["first_frame"],
            t["last_frame"],
            t["n_dets"],
    ...
    logger.info(
        "  Total rows: %d  Cup rows: %d (%d interpolated)  Unique cup IDs: %s",
        len(df),
        len(relinked_cups),
    ...
    for tid in tids_after:
        ...
        is_pour = (
            n_frames >= min_pour_frames and movement > movement_threshold and not is_stationary
        )

        if is_stationary and n_frames >= min_pour_frames:
            status = f"skip (stationary {frac_stationary:.0%})"
        elif is_pour:
            status = "POUR"
        else:
            status = "skip"
        ...
        logger.debug(
            "  Cup %3d: %.1fs->%.1fs  frames=%d  move=%.1fpx  stationary=%.0f%%  -> %s",
            tid,
            time_start,
            time_end,
            n_frames,
    ...
    ax2.set_title(
        f"AFTER re-linking — {n_after} physical cups, "
    ...
    for tid in tids_after:
        ...
        ax_x.plot(sub["frame"], sub["cx"], marker=".", markersize=3, label=f"Cup {tid}")
        ax_y.plot(sub["frame"], sub["cy"], marker=".", markersize=3, label=f"Cup {tid}")
    ...
    if pour_events and video_path and video_path.exists():
        ...
        logger.info(
            "Auto-video: %d segment(s) with %.1fs padding", len(merged_ranges), video_padding
        )
        for i, (s, e) in enumerate(merged_ranges):
            logger.debug(
                "  Segment %d: frames %d-%d (%.1fs -> %.1fs)", i, s, e, s / fps_est, e / fps_est
            )

        render_annotated_video(
            video_path, output_dir, merged_ranges, df, filename="relinked_pour_clip.mp4"
        )
