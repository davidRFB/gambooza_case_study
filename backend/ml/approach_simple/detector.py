...
def _process_chunk(args: tuple) -> tuple:
    ...
    """
    (video_path, start_frame, end_frame, sample_every, blur_kernel,
     diff_threshold, tap_a_px, tap_b_px) = args
        ...
        if frame_idx % sample_every == 0:
            ...
            if prev_gray is not None:
                ...
                if crop_a_rgb is None:
                    crop_a_rgb = cv2.cvtColor(
                        frame[ay1:ay2, ax1:ax2], cv2.COLOR_BGR2RGB
                    )
                    crop_b_rgb = cv2.cvtColor(
                        frame[by1:by2, bx1:bx2], cv2.COLOR_BGR2RGB
                    )
    ...
    cap.release()
    return (frame_indices, signals_a, signals_b,
            hmap_a, hmap_b, hmap_count, crop_a_rgb, crop_b_rgb)
...
class SimpleDetector:
    ...
    def run(self, video_path: str | Path) -> SimpleDetectorResult:
        ...
        print(f"  {vid_w}x{vid_h} | {fps:.1f} fps | {total_frames} frames | "
        ...
        if use_mp:
            result_parts = self._run_multiprocess(
                str(video_path), total_frames, tap_a_px, tap_b_px
            )
        else:
            result_parts = self._run_single(
                str(video_path), total_frames, tap_a_px, tap_b_px
            )

        (frame_indices, sig_a, sig_b,
         hmap_a, hmap_b, hmap_count,
         crop_a_rgb, crop_b_rgb) = result_parts
        ...
        print(f"  Processed {len(frame_indices)} frame pairs | "
    ...
    def _run_single(self, video_path, total_frames, tap_a_px, tap_b_px):
        ...
        while True:
            ...
            if show_progress and frame_idx % self.progress_every == 0:
                ...
                print(f"\r  Progress: {frame_idx}/{total_frames} "
    ...
    def _run_multiprocess(self, video_path, total_frames, tap_a_px, tap_b_px):
        ...
        for i in range(n):
            ...
            chunks.append((
                video_path, overlap_start, end, self.sample_every,
                self.blur_kernel, self.diff_threshold, tap_a_px, tap_b_px,
            ))
        ...
        return (all_indices, all_sig_a, all_sig_b,
                hmap_a, hmap_b, total_hmap_count,
                first_crop_a, first_crop_b)
    ...
    def _detect_events(
            ...
            if is_on and not in_event:
                ...
                if length >= self.min_on_frames:
                    events.append(self._make_event(
                        tap, start_idx, i - 1, signal, frame_indices, fps
                    ))
        ...
        if in_event:
            ...
            if length >= self.min_on_frames:
                events.append(self._make_event(
                    tap, start_idx, len(signal) - 1, signal,
                    frame_indices, fps
                ))
    ...
    def _make_event(
        self, tap, start_idx, end_idx, signal, frame_indices, fps
    ) -> TapEvent:
...
def plot_simple_results(
    ...
    has_heatmaps = (result.heatmaps.get("Tap A") is not None and
                    result.crop_frames.get("Tap A") is not None)
    if has_heatmaps:
        ...
        plt.colorbar(im, ax=[ax_a, ax_b], fraction=0.046,
                     label="Activity fraction")
        fig.suptitle("Tap handle activity heatmaps", fontsize=13)
        fig.tight_layout()
        fig.savefig(output_dir / "tap_heatmaps.png", dpi=150,
                    bbox_inches="tight")
        plt.close(fig)
    ...
    if len(result.times) > 0:
        ...
        for ax, label, color, tap_key in [
            ...
            ax.axhline(y=on_threshold, color="gray", linestyle="--",
                       linewidth=0.8, alpha=0.5, label="ON threshold")
            for e in result.events:
                if e.tap == tap_key:
                    ax.axvspan(e.timestamp_start, e.timestamp_end,
        ...
        fig.tight_layout()
        fig.savefig(output_dir / "tap_signals.png", dpi=150,
                    bbox_inches="tight")
        plt.close(fig)
